...
userRoutes.post('/user/login/password', async (req, res) => {
  ...
  try {
    ...
    let result = await knex.raw(
      'select id, is_admin, password_hash from "user" where email = ?',
      [req.body.username],
    )
    rows = result.rows
  } catch (error: any) {
...
userRoutes.get('/user/login/google', async (req, res) => {
  ...
  let result = await knex.raw(
    'select id, is_admin from "user" where email = ?',
    [userInfo.email],
  )
  if (result.rowCount == 0) {
    // new user
    result = await knex.raw(
      'insert into "user" (email, avatar) values (?, ?) returning id',
...
userRoutes.get('/user', async (req, res) => {
  ...
  try {
    result = await knex.raw('select avatar from "user" where id = ?', [
      user.id,
    ])
  } catch (error) {
