...
async function importUser(user: InsertRow<User>) {
  ...
  if (result.rowCount == 0) {
    await client.query(
      `insert into "user" (username, password, is_admin, ban_reason) values ($1, $2, $3, $4)`,
      [user.username, user.password, user.is_admin, user.ban_reason],
    )
  } else {
...
async function importMemo(memo: InsertRow<Memo>) {
  ...
  await client.query(
    ...
    [memo.content, memo.photo, memo.user_id],
  )
}
...
async function main() {
  ...
  let userDataFile = new DataFile<User[]>('user-list.json', [])
  let memoDataFile = new DataFile<Memo[]>('memo-list.json', [])
